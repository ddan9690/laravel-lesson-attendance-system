...
<div class="overflow-x-auto">
    <table class="min-w-full bg-white border border-gray-200 table-auto">
        <thead class="bg-school-green text-white">
            <tr>
                <th class="py-2 px-4 text-left whitespace-nowrap">{{ $type === 'forms' ? 'Form Name' : 'Grade' }}</th>
        ...
        <tbody>
            ...
            <tr x-data="{
                    supervisor: '{{ $item->class_supervisor_id ?? '' }}',
                    saving: false,
                    async updateSupervisor() {
                        const result = await Swal.fire({
                            title: 'Are you sure?',
                            text: 'This will update the class supervisor.',
                            icon: 'warning',
                        ...
                        this.saving = true;
                        try {
                            const res = await fetch('{{ route('classes.updateSupervisor') }}', {
                                method: 'POST',
                                headers: {
                                ...
                                body: JSON.stringify({
                                    ...
                                    {{ $type === 'forms' ? 'form_id' : 'grade_id' }}: {{ $item->id }}
                                })
                            });
                ...
                <td class="py-2 px-4 whitespace-nowrap">
                    <select x-model="supervisor" @change="updateSupervisor"
